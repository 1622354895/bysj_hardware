...
u16 GetGP2YSingleValue(void)
{
    ...
    ADCVal = GPY2_Get_Adc_Aveurage(ADC_Channel_1,5);  //PA1 采样，读取AD值,取平均 这个才是正确的
    ...
    //Voltage = (float)ADCVal / 4095 * 5.0; //获得AO输出口的电压值
...
u16 GPY2_Get_Adc_Aveurage(u8 ch,u8 times)
{
	...
	for(t=0;t<times;t++)
	{
        GPIO_ResetBits(GPIOA, GPIO_Pin_5);//置1  开启内部LED
        delay_us(280);
		pm_val+=GP2Y_Get_Adc(ch);
        delay_us(40);
         GPIO_SetBits(GPIOA, GPIO_Pin_5);//置1  开启内部LED
       delay_us(9680);
		delay_us(5);
	}
